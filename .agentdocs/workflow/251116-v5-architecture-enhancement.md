...
### 阶段 1: 工具契约规范（预计 2h）
- [x] 创建任务文档
- [ ] 设计工具契约模板（JSON Schema）
- [ ] 编写 search_data_sources 完整契约
- [ ] 编写 filter_data 完整契约（含分页/采样）
- [ ] 编写 fetch_private_data 完整契约
- [ ] 编写其他工具契约
- [ ] 定义错误码规范
- [ ] 更新 V5.0 文档（新增 4.4 章节）

### 阶段 2: 授权与隐私治理（预计 1.5h）
- [ ] 设计 OAuth Token 管理方案
- [ ] 设计跨租户数据隔离方案
- [ ] 设计敏感数据遮罩规则
- [ ] 设计权限拒绝与降级策略
- [ ] 定义审计日志要求
- [ ] 更新 V5.0 文档（新增 4.5 章节）

### 阶段 3: 知识图谱存储方案（预计 1h）
- [ ] 确定存储介质（内存 -> Redis -> 图数据库）
- [ ] 设计容量与 TTL 治理
- [ ] 设计并发控制策略
- [ ] 设计失败回滚策略
- [ ] 说明与 V4.4 兼容性
- [ ] 更新 V5.0 文档（修订 5.4 章节）
...
- [ ] 定义 Planner 协议
- [ ] 定义切换条件
- [ ] 更新 V5.0 文档（修订 5.2 章节）

### 阶段 5: 重排阶段目标（预计 0.5h）
- [ ] 将 compare_data 移至 P0
- [ ] 将 aggregate_data 移至 P1
- [ ] 为每阶段添加验收标准
- [ ] 为每阶段添加回退开关
- [ ] 更新 V5.0 文档（修订第 8 章）
