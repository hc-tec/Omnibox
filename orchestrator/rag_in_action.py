...
from rag_system.config import RETRIEVAL_CONFIG
...
class RAGInAction:
    ...
    def process(
        ...
        try:
            ...
            # ========== 低置信度检测 ==========
            # 检查最高相似度分数，如果低于高置信度阈值则提前返回
            high_confidence_threshold = RETRIEVAL_CONFIG.get("high_confidence_threshold", 0.7)
            top_score = max(tool.get("score", 0) for tool in retrieved_tools)

            if top_score < high_confidence_threshold:
                logger.warning(
                    f"RAG 检索置信度过低: 最高分={top_score:.3f}, 阈值={high_confidence_threshold}. "
                    f"查询: {user_query}"
                )

                # 构建候选工具的简要描述，供用户参考
                candidates_desc = self._format_low_confidence_candidates(retrieved_tools[:3])

                return {
                    "status": "low_confidence",
                    "reasoning": f"RAG 检索到的最相关结果相似度仅为 {top_score:.1%}，低于置信度阈值 {high_confidence_threshold:.0%}。"
                                 f"系统无法确定是否有匹配的数据源。",
                    "generated_path": None,
                    "selected_tool": None,
                    "parameters_filled": {},
                    "clarification_question": f"我没有找到与您的查询非常匹配的数据源。{candidates_desc}"
                                              f"\n\n请尝试：\n"
                                              f"1. 更具体地描述您想查询的内容（如指定平台名称）\n"
                                              f"2. 使用不同的关键词重新描述\n"
                                              f"3. 如果上述候选数据源中有您需要的，请明确告诉我",
                    ...
                    if selected_route_def:
                        ...
                        try:
                            ...
                            if original_params:
                                ...
                                missing_identifiers = self._detect_missing_identifiers(
                                ...
                                if missing_identifiers:
                                    ...
                                    clarification = self._build_identifier_clarification(
                                        route_def=selected_route_def,
                                        missing_params=missing_identifiers,
                                        original_params=original_params,
                                    )
                                    parse_result["status"] = "needs_clarification"
                                    parse_result["generated_path"] = None
                                    parse_result["reasoning"] = (
                                        f"缺少关键标识符：{', '.join(missing_identifiers)}。请提供对应的 ID。"
                                    )
                                    existing_question = parse_result.get("clarification_question")
                                    if existing_question:
                                        clarification = f"{existing_question}\n{clarification}"
                                    parse_result["clarification_question"] = clarification

                                self._maybe_enrich_search_keywords(parse_result, user_query)
    ...
    @staticmethod
    def _format_low_confidence_candidates(tools: List[Dict[str, Any]]) -> str:
        """
        格式化低置信度候选工具的描述，供用户参考。

        Args:
            tools: 候选工具列表（已按相似度排序）
        ...
        for i, tool in enumerate(tools, 1):
            name = tool.get("name") or tool.get("route_id", "未知")
            datasource = tool.get("datasource") or tool.get("provider", "")
            ...
            # 格式化单个工具的描述
            if datasource:
                lines.append(f"{i}. 【{datasource}】{name}（匹配度: {score:.0%}）")
            else:
                lines.append(f"{i}. {name}（匹配度: {score:.0%}）")
    ...
    @staticmethod
    def _normalize_parameter_definitions(route_def: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """将 parameter 定义统一为 {name: definition} 格式。"""
        normalized: Dict[str, Dict[str, Any]] = {}
        params = route_def.get("parameters") or {}
    ...
    @staticmethod
    def _collect_required_identifiers(
        route_def: Dict[str, Any],
        resolution_status: Dict[str, bool],
    ) -> List[str]:
        """收集需要解析或补充的标识符字段。"""
        required = [
            name for name in route_def.get("required_identifiers") or []
            if isinstance(name, str) and name
        ]

        param_defs = RAGInAction._normalize_parameter_definitions(route_def)
        if not required:
            for name, definition in param_defs.items():
                param_type = definition.get("parameter_type")
                if param_type == "entity_ref":
                    required.append(name)
                elif not param_type and definition.get("required"):
                    required.append(name)

        if not required and resolution_status:
            required.extend(resolution_status.keys())
    ...
    def _detect_missing_identifiers(
        self,
        route_def: Dict[str, Any],
        ...
        required = self._collect_required_identifiers(route_def, resolution_status)

        missing: List[str] = []
        for name in required:
            if not name:
                continue
            resolved_state = resolution_status.get(name)
            value = parameters.get(name)
            if not value or resolved_state is False:
                missing.append(name)
        return missing

    @staticmethod
    def _build_identifier_clarification(
        route_def: Dict[str, Any],
        missing_params: List[str],
        original_params: Optional[Dict[str, Any]],
    ) -> str:
        """生成提示用户补充标识符的澄清问题。"""
        ...
        route_name = (
            route_def.get("name")
            or route_def.get("description")
            or route_def.get("route_id")
            or "该功能"
        )
        platform = (
            route_def.get("datasource_name")
            or route_def.get("provider_name")
            or route_def.get("datasource")
            or ""
        )

        def describe_param(param: str) -> str:
            definition = param_defs.get(param) or {}
            display = (
            ...
            return f"{display}（{description}）" if description else display

        pretty_names = "、".join(describe_param(param) for param in missing_params if param)
        if not pretty_names:
            pretty_names = "必要标识符"

        message = f"要获取{route_name}，需要提供{platform or '对应数据源'}的{pretty_names}。"
        if missing_params:
            sample_value = original_params.get(missing_params[0])
            if sample_value:
                message += f" 当前识别到的是“{sample_value}”，但尚未解析为有效的 ID。"
        message += " 请提供准确的标识符（例如可粘贴该对象主页中的数字 UID 或完整链接）。"
        return message

    def _maybe_enrich_search_keywords(self, parse_result: Dict[str, Any], user_query: str) -> None:
        """当用户指定了 UP 主线索时，尝试把名称并入搜索关键词，提升命中率。"""
        selected_tool = parse_result.get("selected_tool") or {}
        route_id = (selected_tool.get("route_id") or "").lower()
        if route_id not in {"bilibili_vsearch"}:
            return

        params = parse_result.get("parameters_filled") or {}
        keyword = params.get("kw")
        if not keyword or not isinstance(keyword, str):
            return

        up_hint = self._extract_up_name_hint(user_query)
        if not up_hint or up_hint in keyword:
            return

        enriched_keyword = f"{up_hint} {keyword}".strip()
        params["kw"] = enriched_keyword
        parse_result["parameters_filled"] = params
        logger.info("🎯 关键词增强：检测到 UP 主线索 '%s'，搜索词更新为 '%s'", up_hint, enriched_keyword)

    @staticmethod
    def _extract_up_name_hint(user_query: str) -> Optional[str]:
        """从查询语句中提取 B 站 UP 主名称的启发式辅助函数。"""
        if not user_query:
            return None

        normalized = user_query.strip()
        patterns = [
            r"(?:B站|哔哩哔哩|bilibili)\s*([^\s,，。、“”\"'的]{1,20})(?=(?:的|投稿|UP|频道|空间|作品))",
            r"(?:UP主|UP)\s*([^\s,，。、“”\"'的]{1,20})",
        ]
    ...
    def _print_result(self, result: Dict[str, Any]) -> None:
        ...
        if result['status'] == 'success':
            ...
            print(f"\n[低置信度] RAG 检索结果置信度不足:")
            print(f"  最高匹配度: {result.get('top_score', 0):.1%}")
            print(f"  原因: {result.get('reasoning')}")
            retrieved = result.get('retrieved_tools', [])
            if retrieved:
                print(f"  候选工具数: {len(retrieved)}")
